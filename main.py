...
torch.backends.cudnn.benchmark = False#保证每次测试结果一样
...
# 数据准备：按照 8:1 的比例划分训练集和验证集
train_sequences, val_sequences = train_test_split(
    ner_data,
    test_size=0.1,  # 10% 的数据用于验证
    random_state=42,  # 设置随机种子，确保可重复性
)

print(f"训练集大小: {len(train_sequences)}, 验证集大小: {len(val_sequences)}")
print("Training Sequences:")
print(train_sequences["text"])
print("Validation Sequences:")
print(val_sequences["text"]) 
...
for eo in range(EPOCH):  # epoch
    ...
    #这些指标是根据当前 epoch 计算出的，并且在每个 epoch 的结束时计算并输出
    f1fun.reset()
    recallfun.reset()
    precisionfun.reset()
    ...
    with torch.no_grad():
        ...
        for idx, batch in tqdm.tqdm(enumerate(ner_loader_val)):
            ...
            #用f1all1位作为指标，寻找最佳阈值
            #在不同的阈值下计算多个评估指标（F1 分数、召回率、精确率、特异度、准确率和 AUC）
            #然后找出使 F1 分数最大的阈值，这个阈值即为最佳阈值
            f1all1 = 0
        ...
        #在模型训练的输出信息中，aucmean1和aucevalmean两个指标的数值其实是一样的
